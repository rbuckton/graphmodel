...
export class GraphPropertyCollection<P extends object = any> {
    ...
    private _properties = new Map<string, GraphProperty<keyof P, P[keyof P]>>();
    ...
    public get<K extends keyof P>(id: K) {
        return this._properties.get(id) as GraphProperty<K, P[K]> | undefined;
    }

    public getOrCreate<K extends keyof P>(id: K) {
        let property = this._properties.get(id);
        if (!property) this.add(property = GraphProperty.create(id));
        return property as GraphProperty<K, P[K]>;
    }

    public add<K extends keyof P>(property: GraphProperty<K, P[K]>) {
    ...
    /*@internal*/ values<K extends keyof P>(propertyIds: Iterable<K>): IterableIterator<GraphProperty<K, P[K]>>;
    public values(): IterableIterator<GraphProperty<keyof P, P[keyof P]>>;
    ...
    private _raiseOnAdded(property: GraphProperty<keyof P, P[keyof P]>) {
        for (const { onAdded } of this._observers.values()) if (onAdded) onAdded(property);
    }

    private _raiseOnDeleted(property: GraphProperty<keyof P, P[keyof P]>) {
